...
pub fn handler(ctx: Context<JudgeUnregister>) -> Result<()> {
    ...
    require!(current_time > ctx.accounts.judge_record.locked_until, TaskError::StakeLocked);
    ...
    let last_idx = registry.active_count.checked_sub(1).ok_or(TaskError::MathOverflow)? as usize;
    registry.judges[remove_idx] = registry.judges[last_idx];
    registry.judges[last_idx] = Pubkey::default();
    registry.active_count = registry.active_count.checked_sub(1).ok_or(TaskError::MathOverflow)?;

    **ctx.accounts.judge_stake_vault.to_account_info().try_borrow_mut_lamports()? = ctx.accounts.judge_stake_vault
        .to_account_info()
        .lamports()
        .checked_sub(amount_to_refund)
        .ok_or(TaskError::MathOverflow)?;
    **ctx.accounts.judge.try_borrow_mut_lamports()? = ctx.accounts.judge
        .lamports()
