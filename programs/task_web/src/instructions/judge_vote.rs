...
pub fn handler(ctx: Context<JudgeVote>, is_pass: bool) -> Result<()> {
    ...
    let is_assigned = task.assigned_judges.iter().any(|&key| key == judge_key);
    require!(is_assigned, TaskError::NotAssignedJudge);

    let current_time = Clock::get()?.unix_timestamp;
    require!(current_time <= task.voting_deadline, TaskError::DeadlinePassed);
    ...
    if is_pass {
        task.pass_vote_count = task.pass_vote_count
            .checked_add(1)
            .ok_or(TaskError::MathOverflow)?;
    } else {
        task.fail_vote_count = task.fail_vote_count
            .checked_add(1)
            .ok_or(TaskError::MathOverflow)?;
    }
