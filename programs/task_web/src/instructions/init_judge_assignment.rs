...
#[derive(Accounts)]
pub struct InitJudgeAssignment<'info> {
    //acc cho 1 judge thực tiện 1 task
    #[account(mut)]
...
pub fn handler(ctx: Context<InitJudgeAssignment>) -> Result<()> {
    ...
    require!(judge_key != Pubkey::default(), TaskError::NotAssignedJudge);

    let assigned_order = task
        .assigned_judges //Tìm vị trí của judge trong danh sách assigned_judges của task để biết được thứ tự được assign vào task này
        .iter()
        .take(task.assigned_judge_count as usize)
